...
class CoffeeRoasterApp {
    ...
    async init() {
        // Pokaż skeleton tylko w listach
    ...
    async initSupabase() {
        try {
            if (!window.SUPABASE_CONFIG || window.SUPABASE_CONFIG.url === 'TU_WKLEJ_URL_PROJEKTU' || window.SUPABASE_CONFIG.anonKey === 'TU_WKLEJ_ANON_KEY') {
    ...
    attachStageListeners() {
        document.querySelectorAll('.btn-remove-stage').forEach(btn => {
            ...
            newBtn.addEventListener('click', (e) => {
                ...
                if (rows.length > 1) { row.remove(); document.querySelectorAll('.stage-row').forEach((r, i) => { r.querySelector('.stage-num').textContent = i + 1; r.dataset.stage = i + 1; }); }
                else this.showToast('Musi zostać przynajmniej jeden etap', 'warning');
            });
        });
    }
    ...
    openProfileViewModal(profile) {
        ...
        let stagesHTML = ''; let hasFCStage = false;
    ...
    startRoastingMode(profileId) {
        const profile = this.profiles.find(p => p.id === profileId);
        if (!profile) return;
        this.roastingProfile = profile; this.roastingTime = 0; this.roastingFCClicked = false; this.roastingActualFCTime = null; this.lastStageIndex = 0;
    ...
    startRoastingTimer() {
        this.requestWakeLock(); this.roastingPaused = false;
    ...
    playStageChangeSound() {
        try {
            ...
            playTone(1319, now, 0.15); playTone(1047, now + 0.12, 0.2);
        } catch (e) {}
    }
    ...
    toggleRoastingPause() {
        ...
        if (this.roastingPaused) {
            this.roastingPaused = false; this.requestWakeLock();
            if (this.noSleep) this.noSleep.enable();
            this.roastingInterval = setInterval(() => { this.roastingTime++; this.updateRoastingDisplay(); this.checkStagesProgress(); }, 1000);
            btn.innerHTML = '⏸️ Pauza'; btn.classList.remove('paused');
        } else {
            this.roastingPaused = true;
            if (this.roastingInterval) { clearInterval(this.roastingInterval); this.roastingInterval = null; }
            this.releaseWakeLock(); this.stopIosWakeLock();
            btn.innerHTML = '▶️ Wznów'; btn.classList.add('paused');
        }
    }

    recordRoastingFC() {
        if (this.roastingFCClicked) return;
        this.roastingFCClicked = true; this.roastingActualFCTime = this.roastingTime;
    ...
    // ===== STOPER =====
    setupStopwatch() {
        this.stopwatchInterval = null; this.stopwatchTime = 0; this.stopwatchRunning = false; this.stopwatchSticky = false;
    ...
    setupStickyStopwatch() {
        const stopwatch = this.stopwatchEl(); const placeholder = this.placeholderEl();
    ...
    updateStopwatchDisplay() {
        ...
        if (display) { const h = Math.floor(this.stopwatchTime / 3600); const m = Math.floor((this.stopwatchTime % 3600) / 60); const s = this.stopwatchTime % 60; display.textContent = `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`; }
    ...
    addFirstCrackStage() {
        const container = document.getElementById('roastStages'); if (!container) return;
        const mins = Math.floor(this.stopwatchTime / 60); const secs = this.stopwatchTime % 60;
        const timeStr = mins.toString().padStart(2,'0') + ':' + secs.toString().padStart(2,'0');
        const fcStage = document.createElement('div');
        fcStage.className = 'stage-row stage-fc'; fcStage.dataset.stage = 'fc';
        ...
        container.appendChild(fcStage); this.attachStageListeners(); fcStage.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }

    updateFirstCrackPercent() {
        const percentEl = document.getElementById('firstCrackPercent');
        if (percentEl && this.firstCrackTime !== null) {
            if (this.stopwatchTime > 0 && this.firstCrackTime > 0) { const timeAfterFC = this.stopwatchTime - this.firstCrackTime; percentEl.textContent = ((timeAfterFC / this.stopwatchTime) * 100).toFixed(1) + '%'; }
